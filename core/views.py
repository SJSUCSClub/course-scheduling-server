...
def sql_courses(request, course):
    ...
    if csn and dept:
        ...
        # print("What does this return", course_reviews)
        # json_data = row_merge([json_data, course_reviews])
    else:
...
def prof_auxiliary_json(prof_id):
    with connection.cursor() as cursor:
        ...
        # test_query = """select count(*) from reviews where department='CS' and course_number='152'
        #     group by ease;"""

        # cursor.execute(test_query)
        # tmp = cursor.fetchall()
        # print([int(x[0]) for x in tmp])
