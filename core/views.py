...
def pull_reviews(csn, dept, comments, page=None, limit=None, tags=None):
    ...
    init_json = where("Reviews", {"course_number": csn, "department": dept}, tags=tags)
    ...
    for json_obj in json_data:
        ...
        if json_obj["id"] is not None:
            ...
            critics = where(
                "user_review_critique", {"review_id": json_obj["id"]}, ["upvote"]
            )
            ...
            if (
                comments
                and comments != False
                and comments.capitalize() != "False"
                and comments.capitalize() == "True"
            ):
                comments = where("comments", {"review_id": json_obj["id"]})
                json_obj = row_merge(
        ...
        if json_obj["professor_id"] is not None:
            prof = where("users", {"id": json_obj["professor_id"]}, ["name", "email"])
        ...
        if json_obj["user_id"] is not None:
            user = where("users", {"id": json_obj["user_id"]}, ["username", "name"])
...
def sql_courses(request, course):
    try:
        ...
        if csn and dept:
            json_data = where("Courses", {"course_number": csn, "department": dept})
            if len(json_data) > 0:
...
def sql_schedules(request, course):
    try:
        ...
        if csn is not None and dept is not None:
            ...
            final_resp["pages"] = len(full_json) // limit + (
                1 if len(full_json) % limit > 0 else 0
            )
...
def course_summary_controller(request, course):
    try:
        ...
        if csn is not None and dept is not None:
            ...
            return JsonResponse(
                {"message": " course_number and or department not specified"},
                status=status.HTTP_404_NOT_FOUND,
            )
...
def prof_pull_reviews(prof_id, comments, page=None, limit=None, tags=None):
    ...
    json_data = where(
        "Reviews", {"professor_id": prof_id}, page=page, limit=limit, tags=tags
    )

    init_json = where("Reviews", {"professor_id": prof_id}, tags=tags)
...
def sql_professors(request, professor_id):
    try:
        ...
        if prof_id:
            json_data = where("users", {"id": prof_id, "is_professor": True})
            if len(json_data) > 0:
...
def professor_sql_schedules(request, professor_id):
    try:
        ...
        if prof_id is not None:
            ...
            json_data = where(
                "Schedules", {"professor_id": prof_id}, page=page, limit=limit
            )
            ...
            for json_obj in json_data:
                if json_obj["professor_id"] is not None:
                    ...
                    course_title = where(
                        "Courses",
                        {
                            "course_number": json_obj["course_number"],
        ...
        if json_data is None:
            return JsonResponse(
                {"message": "An error occurred"}, status=status.HTTP_404_NOT_FOUND
            )
        return JsonResponse(final_resp, safe=False)
    except:
        return JsonResponse({"message": "An error occurred"}, status=500)
...
def prof_summary(request, professor_id):
    try:
        ...
        if prof_id is not None:
            json_data = where("users", {"id": prof_id})
            ...
            return JsonResponse(
                {"message": " professor_id not specified"},
                status=status.HTTP_404_NOT_FOUND,
            )
    except Exception as e:
...
def prof_reviews(request, professor_id):
    ...
    try:
        ...
        count_query = (
            "select count(*) as total_count from reviews where professor_id=%s"
        )
        ...
        data, rows = run_sql(query + "limit %s offset %s", (professor_id, limit, page))
        ...
        for json_obj in json_res:
            ...
            if comments and comments.capitalize() == "True":
                comments = where("comments", {"review_id": json_obj["review_id"]})
...
def course_search(request):
    try:
        ...
        if search:
            ...
            count_data, _ = run_sql(
                "SELECT COUNT(*) FROM courses WHERE department = %s", (dept,)
            )
            data, rows = run_sql(
                "SELECT * FROM courses WHERE department = %s LIMIT %s OFFSET %s",
                (
                    dept,
                    limit,
                    (int(page) - 1) * limit,
            ...
            final_results["pages"] = count_data[0][0] // limit + (
                1 if count_data[0][0] % limit != 0 else 0
            )
            final_results["page"] = page
            final_results["items"] = data
            final_results["filters"] = {
                "departments": [dept] if count_data[0][0] > 0 else []
            }
        else:
            # no search or department specified, so just select *
            # make queries
            department_data, department_rows = run_sql(
                "SELECT DISTINCT department FROM courses"
            )
            count_data, _ = run_sql("SELECT COUNT(*) FROM courses")
            paged_data, rows = run_sql(
...
def prof_search(request):
    try:
        ...
        if search:
            ...
            final_results["pages"] = count[0][0] // limit + (
                1 if count[0][0] % limit != 0 else 0
            )
            final_results["page"] = page
            final_results["items"] = data
...
def schedule_search(request):
    try:
        ...
        uterms, uyears, uprof_names, ucn, udept, umoi, uunits = (
            set(),
            set(),
            set(),
            set(),
